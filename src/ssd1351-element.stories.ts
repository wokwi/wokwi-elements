...
function toImageData(bitmap: string[], width: number, height: number) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      result.data[pixOffset + 0] = (bitmap[pixIndex] & 0xff0000) >> 16;
      result.data[pixOffset + 1] = (bitmap[pixIndex] & 0x00ff00) >> 8;
      result.data[pixOffset + 2] = bitmap[pixIndex] & 0x0000ff;
...
storiesOf('SSD1351', module)
  ...
  .add(
    'Wokwi Logo',
    () => html`<wokwi-ssd1351 .imageData=${toImageData(wokwiLogo, 128, 128)}></wokwi-ssd1351>`
  );
